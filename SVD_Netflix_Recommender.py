...
# data = tf.convert_to_tensor(users_movies)
# s, u, v = tf.linalg.svd(data, full_matrices=True, compute_uv=True)
# s = tf.linalg.diag(s, num_rows=data.shape[0], num_cols=data.shape[1])
# UDV(^T)
# b = tf.tensordot(tf.tensordot(u, s, axes=1), tf.transpose(v), axes=1).numpy()

# using numpy
# a = users_movies.to_numpy()
# U, S, VT = svd(a)
...
def SVD_MF(biased, epochs, factors):
    P_nap_final = P_nap.copy()
    # using surprise
    reader = Reader(rating_scale=(0.5, 5))
    data_2 = Dataset.load_from_df(df_ratings, reader)
    algo = SVD(biased=biased, n_epochs=epochs, n_factors=factors)
    trainset = data_2.build_full_trainset()
    testset = trainset.build_anti_testset(fill=0)
    model = algo.fit(trainset)
    predictions = model.test(testset)
    accuracy.rmse(predictions, verbose=True)
    df = pd.DataFrame(predictions, columns=['user', 'item', 'rating', 'est_rating', 'was_impossible'])
    for i in range(len(df)):
        P_nap_final.values[P_nap.index.get_loc(df.item[i]), df.user[i]] = df.est_rating[i]
    movie_id = 0
    new = P_nap_final.copy()
    ...
    while movie_id < len(users_movies.index):
        ...
        while user_id < len(users_movies.columns):
            if users_movies.values[movie_id][user_id] == P_nap_final.values[movie_id][user_id]:
