...
# enumerate movie and user Ids
# trainset_df['userId'] = trainset_df['userId'].apply(lambda x: userid2idx[x])
# trainset_df['movieId'] = trainset_df['movieId'].apply(lambda x: movieid2idx[x])

# testset_df['userId'] = testset_df['userId'].apply(lambda x: userid2idx[x])
# testset_df['movieId'] = testset_df['movieId'].apply(lambda x: movieid2idx[x])
...
n_movies = len(trainset_df['movieId'].unique())
n_users = len(trainset_df['userId'].unique())
n_latent_factors = 50  # hyperparamter to deal with.

# user embedding
user_input = Input(shape=(1,), name='user_input', dtype='int64')
user_embedding = Embedding(n_users, n_latent_factors, name='user_embedding')(user_input)
# user_embedding = Embedding(input_dim=n_users, output_dim=50, name='user_embedding', embeddings_regularizer=tf.keras.regularizers.L2(0.05), input_length=1)
...
# movie_embedding = Embedding(input_dim=n_movies, output_dim=50, name='item_embedding', embeddings_regularizer=tf.keras.regularizers.L2(0.05), input_length=1)
...
# movies_bias_vec = Flatten()(movies_bias(movie_input))
...
# model.summary()
plot_model(nn_model, to_file="nn_model_embeddings.png", show_shapes=True, show_layer_names=True)
nn_model.compile(optimizer=Adam(lr=1e-3), loss=root_mean_squared_error,metrics=[precision_m,'acc',recall_m])
