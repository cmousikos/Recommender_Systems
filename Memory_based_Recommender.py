...
target_movie = 0
w = len(users_movies.index)
cosine_similarity = np.empty(shape=(w,w)) #compute only the diagonal
while target_movie < len(users_movies.index):
    similarMovie = 0
        ...
        while user_id < len(array.columns):
            ...
            if array.values[movie_id][user_id] == 0:
                for l in range(10):
                    ...
                    numerator = (cosine_similarity[idx][movie_id]) * (array.values[idx][user_id])
                    temp = temp + numerator
                    temp_2 = temp_2 + cosine_similarity[movie_id][k]
                    k = k + 1
            if (temp > 0) & (temp_2 > 0):
...
def recommend_10_highest(array):
    ...
    return new
P_nap = recommender(users_movies)
